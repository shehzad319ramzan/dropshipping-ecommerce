      ...
      <div className="container relative z-10">
        <div className="mb-12 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <h2 className="mt-5 text-4xl font-black tracking-tight text-slate-900 sm:text-5xl dark:text-slate-50">
              Best picks to shop first
            </h2>
          ...
          <Link
            href="/shop"
            className="group inline-flex items-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-bold text-slate-900 shadow-md ring-1 ring-slate-900/5 transition-all hover:bg-slate-50 hover:shadow-lg dark:bg-slate-900 dark:text-white dark:ring-white/10 dark:hover:bg-slate-800"
          >
            View full catalogue
...
function ProductShowcaseCard({ product }) {
  ...
  useEffect(() => {
    setSoldCount(Math.floor(Math.random() * 50) + 10)
  }, [])
  ...
  return (
    <article className="group flex flex-col overflow-hidden transition-shadow duration-300 hover:shadow-xl hover:shadow-slate-200/50 bg-white rounded-lg dark:bg-slate-900 w-full">
      <Link
        ...
        className="relative block overflow-hidden aspect-square bg-stone-100 dark:bg-slate-950 w-full"
      >
        <img
          src={product.image}
          alt={product.name}
      ...
      <div className="flex flex-col pt-3 px-1 pb-2">
        ...
        <div className="mt-1 flex items-center justify-between relative">
          <div className="flex items-center flex-wrap gap-x-1.5 gap-y-1">
            <span className="text-[17px] font-bold text-slate-900 dark:text-slate-50">
            ...
            {soldCount !== null && (
              <span className="text-[11px] text-slate-500">{soldCount}K+ sold</span>
            )}
            {discount && (
          ...
          <button
            type="button"
            className="flex h-8 w-8 items-center justify-center rounded-full border border-slate-300 bg-white text-slate-900 transition-colors hover:bg-slate-100 ml-2 shrink-0 shadow-sm"
          >
        ...
        <div className="mt-1.5 flex items-center gap-1">
          <div className="flex text-slate-900 dark:text-slate-100">
            {[...Array(5)].map((_, i) => (
              <svg
                key={i}
                className="w-3 h-3 text-slate-900 fill-current dark:text-slate-100"
                viewBox="0 0 20 20"
              >
