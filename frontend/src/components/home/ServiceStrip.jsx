...
export function ServiceStrip() {
  return (
    <section className="border-y border-stone-200 bg-white dark:border-slate-800 dark:bg-slate-950">
      <div className="container grid gap-0 divide-y divide-stone-200 sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-3 dark:divide-slate-800">
        {serviceHighlights.map((item) => {
          const Icon = iconMap[item.icon]
          return (
            <div key={item.id} className="flex items-center gap-3 px-4 py-5">
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-stone-100 text-brand-green dark:bg-slate-900">
